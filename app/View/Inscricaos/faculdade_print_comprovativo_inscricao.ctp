<?php
	...
	$this->PhpExcel->xls->getActiveSheet()->setCellValue('A2',
		$inscricoesActivas[0]['Turma']['Curso']['UnidadeOrganica']['name']);
	...
	foreach ($inscricoesActivas as $inscricao) {
		...
		$this->PhpExcel->xls->getActiveSheet()->setCellValue('E' . $linha_actual,
			$inscricao['Turma']['ano_curricular']);
		$this->PhpExcel->xls->getActiveSheet()->setCellValue('F' . $linha_actual,
			$inscricao['Turma']['semestre_curricular']);
		$this->PhpExcel->xls->getActiveSheet()->setCellValue('G' . $linha_actual,
			$inscricao['Turma']['Disciplina']['DisciplinaPlanoEstudo'][0]['carga_total']);
		...
		$this->PhpExcel->xls->getActiveSheet()->setCellValue('H' . $linha_actual,
			$inscricao['Turma']['Disciplina']['DisciplinaPlanoEstudo'][0]['creditos']);
		$linha_actual++;
		$i++;
	}
	...
	$this->PhpExcel->addImage(WWW_ROOT . 'img' . DS . 'logo_login_' . Configure::read('OpenSGA.instituicao.sigla') . '.png',
		'A1', 10, 108, 115); //Logotipo
	$this->PhpExcel->addImage(WWW_ROOT . 'img' . DS . 'logo_login_' . Configure::read('OpenSGA.instituicao.sigla') . '.png',
		'A26', 10, 108, 115); //Logotipo
