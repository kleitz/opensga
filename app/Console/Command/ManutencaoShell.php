...
class ManutencaoShell extends AppShell
{
    ...
    public function ajustaInscricaoEconomia()
    {
        ...
        foreach ($inscricaos as $inscricao) {
            ...
            if ($inscricao['Turma']['PlanoEstudo']['ano_criacao'] != 2012) {
                ...
                if (empty($turmaNova)) {
                    $disciplinaPlanoEstudo = $this->Curso->PlanoEstudo->DisciplinaPlanoEstudo->findByPlanoEstudoIdAndDisciplinaId($planoEstudo2012['PlanoEstudo']['id'],
                        $disciplinaId);
                    if (empty($disciplinaPlanoEstudo)) {
                        $disciplinaPlanoActual = $this->Curso->PlanoEstudo->DisciplinaPlanoEstudo->findByPlanoEstudoIdAndDisciplinaId($inscricao['Turma']['plano_estudo_id'],
                            $disciplinaId);
                ...
                $inscricaoExiste = $this->Inscricao->findByAlunoIdAndTurmaId($inscricao['Inscricao']['aluno_id'],
                    $turmaNova['Turma']['id']);
            ...
            foreach ($alunos as $aluno) {
                ...
                if (empty($planoEstudo)) {
                    ...
                    if ($planoEstudo['PlanoEstudo']['ano_criacao'] < 2012) {
                        $planoEstudo2012 = $this->PlanoEstudo->findByCursoIdAndAnoCriacao($aluno['Aluno']['curso_id'],
                            2012);
                        if (!empty($planoEstudo2012)) {
    ...
    {
        $this->Turma->processaPauta('/home/backstageel/Downloads/pauta.xlsx', 64310);
    }


    public function actualiza_provincias()
    {
        App::import('Vendor', 'PHPExcel', ['file' => 'PHPExcel.php']);
        if (!class_exists('PHPExcel')) {
            throw new CakeException('Vendor class PHPExcel not found!');
        ...
        $xls = PHPExcel_IOFactory::load(APP . 'Imports' . DS . 'alunos_todos.xlsx');
        $worksheet = $xls->getActiveSheet();

        $linha_actual = 2;
        foreach ($worksheet->getRowIterator() as $row) {
            $controlador = $worksheet->getCell('F' . $linha_actual)->getCalculatedValue();
            if ($controlador == '') {
                break;
            }
            $control = $worksheet->getCell('A' . $linha_actual)->getCalculatedValue();
            $numeroEstudante = trim($worksheet->getCell('B' . $linha_actual)->getCalculatedValue());
            $naturalidade = trim($worksheet->getCell('J' . $linha_actual)->getCalculatedValue());
            $provincia = trim($worksheet->getCell('L' . $linha_actual)->getCalculatedValue());

            //Mapings
            if ($naturalidade == 'Maputo') {
                $naturalidade = 'Katembe';
            }
            if ($naturalidade == 'Cidade de Maputo') {
                $naturalidade = 'Katembe';
            }
            if (in_array($naturalidade, ['Alemanha'])) {
                $naturalidade = 'Estrangeiro';
            }
            ...
            if ($provincia != '') {
                $provinciaDB = $this->Aluno->Entidade->ProvinciaNascimento->find('first',
                    ['conditions' => ['name LIKE' => '%' . $provincia . '%']]);
                $cidadeDB = $this->Aluno->Entidade->CidadeNascimento->find('first',
                    ['conditions' => ['name LIKE' => '%' . $naturalidade . '%']]);
                if (empty($provinciaDB)) {
                    debug('provincia');
                    die(debug($provincia));
                }
            } else {
                $cidadeDB = $this->Aluno->Entidade->CidadeNascimento->find('first',
                    ['conditions' => ['name LIKE' => '%' . $naturalidade . '%']]);
                $provinciaDB = $this->Aluno->Entidade->ProvinciaNascimento->findById($cidadeDB['CidadeNascimento']['provincia_id']);
            }

            $this->Aluno->Entidade->id = $aluno['Entidade']['id'];
            if ($aluno['Entidade']['provincia_nascimento'] == null) {
                $this->Aluno->Entidade->set('provincia_nascimento', $provinciaDB['ProvinciaNascimento']['id']);
            }
            ...
            $this->out($linha_actual . '----' . $numeroEstudante . '----' . $naturalidade . '---' . $provincia);
            $linha_actual++;
        }
                ...
                foreach ($candidatos as $candidato) {
                    if ($candidato['CandidatoGraduacao']['provincia_nascimento']) {
                        $this->Aluno->Entidade->set('provincia_nascimento',
                            $candidato['CandidatoGraduacao']['provincia_nascimento']);
                        if (!$this->Aluno->Entidade->save()) {
                            debug($this->Aluno->Entidade->validationErrors);
                            die();
                        }
            ...
            if ($aluno['Entidade']['cidade_nascimento'] != null) {
                ...
                foreach ($worksheet->getRowIterator() as $row) {
                    ...
                    if ($controlador == $aluno['Entidade']['cidade_nascimento']) {
                        ...
                        $cidadeDB = $this->Aluno->Entidade->CidadeNascimento->find('first',
                            ['conditions' => ['name LIKE' => '%' . $nomeCidade . '%']]);
                        if (!empty($cidadeDB)) {
                            $this->Aluno->Entidade->id = $aluno['Entidade']['id'];
                            $this->Aluno->Entidade->set('provincia_nascimento',
                                $cidadeDB['CidadeNascimento']['provincia_id']);
                            $this->Aluno->Entidade->set('cidade_nascimento', $cidadeDB['CidadeNascimento']['id']);
                            if (!$this->Aluno->Entidade->save()) {
                                debug($this->Aluno->Entidade->validationErrors);
                            }
